...
{
	int current = 5, i, j, trueInt = 0, numericalValue;
	int instances[8] = {0,0,0,0,0,0,0,0};
	char input[5];
	char array[8][3] = {{'A', 'G', 'F'}, {'B', 'H', 'G'}, {'C', 'F', 'A'}, {'D', 'C', 'D'}, {'E', 'A', 'B'}, {'F', 'E', 'H'}, {'G', 'B', 'D'}, {'H', 'D', 'E'}};
	printf("Ctrl+D will output the current state at any given time.\n");
	...
	while(1) {
		...
		else if(input[0] == 'd' && input[2] != '²' && input[2] != ' ') {
